...
export function parseBlock(parsingContext: ParsingContext): BlockStatementNode {
  ...
  consume(tokens, TokenType.WORD, 'BLOCK');
...
export function parseVarStatement(parsingContext: ParsingContext): VariableStatementNode {
  ...
  consume(tokens, TokenType.WORD, "VAR");

  const varName = consume(tokens, TokenType.WORD).normalizedValue;
...
export function parseConstStatement(parsingContext: ParsingContext): ConstantStatementNode {
  ...
  consume(tokens, TokenType.WORD, 'CONST');

  const constName = consume(tokens, TokenType.WORD).normalizedValue;
...
export function parseReturnStatement(parsingContext: ParsingContext): ReturnStatementNode {
  consume(parsingContext.tokens, TokenType.WORD, "RETURN");
...
export function parseOnStatement(parsingContext: ParsingContext): OnEventStatementNode {
  consume(parsingContext.tokens, TokenType.WORD, "ON");
...
function parseDialogueTree(parsingContext: ParsingContext): DialogueTreeStatementNode {
  ...
  consume(tokens, TokenType.WORD, 'DIALOGUE_TREE');
  const treeId = consume(tokens, TokenType.WORD).normalizedValue;
  ...
  while (tokens.length > 0 && !(tokens[0].normalizedValue === 'END' && tokens[1]?.normalizedValue === 'DIALOGUE_TREE')) {
    ...
    if (currentToken.normalizedValue === 'ROUTE') {
      consume(tokens, TokenType.WORD, 'ROUTE');
      ...
      if (tokens[0].normalizedValue === 'IF') {
        ...
        consume(tokens, TokenType.WORD, 'THEN');
        consume(tokens, TokenType.WORD, 'GOTO');
        ...
        consume(tokens, TokenType.WORD, 'DEFAULT');
        consume(tokens, TokenType.WORD, 'GOTO');
        const targetNodeId = consume(tokens, TokenType.WORD).normalizedValue;
...
function parseDialogueNode(parsingContext: ParsingContext): DialogueNodeStatementNode {
  ...
  consume(tokens, TokenType.WORD, 'NODE');
  const nodeId = consume(tokens, TokenType.WORD).normalizedValue;
  ...
  while (tokens.length > 0 && !(tokens[0].normalizedValue === 'END' && tokens[1]?.normalizedValue === 'NODE')) {
    ...
    if (token.normalizedValue === 'TEXT') {
      consume(tokens, TokenType.WORD, 'TEXT');
      ...
      consume(tokens, TokenType.WORD, 'ON_EXECUTE');
...
function parseChoiceExpression(parsingContext: ParsingContext): ChoiceExpressionNode {
  ...
  consume(tokens, TokenType.WORD, 'CHOICE');
...
export function parseAssignmentExpression(parsingContext: ParsingContext): AssignmentExpressionNode {
  ...
  const { normalizedValue: identifier, value } = consume(parsingContext.tokens, TokenType.WORD);
...
export function parseLogicalExpression(parsingContext: ParsingContext): ExpressionNode {
  ...
  else if (token.type === TokenType.WORD && tokens[1].type === TokenType.ASSIGNMENT_OPERATOR)
    left = parseAssignmentExpression(parsingContext);
  else if ((token.type === TokenType.WORD || token.type === TokenType.LITERAL) && tokens[1].type === TokenType.COMPARISON_OPERATOR)
    left = parseComparisonExpression(parsingContext);
  else if ((token.type === TokenType.WORD || token.type === TokenType.LITERAL) && tokens[1].type === TokenType.ARITHMATIC_OPERATOR)
  ...
  else if (token.type === TokenType.LITERAL || token.type === TokenType.WORD)
...
export function parseStatement(parsingContext: ParsingContext): StatementNode {
  ...
  if (token.type === TokenType.FUNCTION) return parseCallExpression(parsingContext);
  if (token.type === TokenType.WORD) return parseAssignmentExpression(parsingContext);
